...
class RNNCell(object):
    ...
    def forward(self, x, h_prev_t):
        ...
        """
        # Wihxt = np.dot(self.W_ih, x.T)
        # Whhht_minus_1 = np.dot(self.W_hh, h_prev_t.T)
        


        h_t = self.activation.forward( np.dot(self.W_ih, x.T) + self.b_ih[:, np.newaxis] + np.dot(self.W_hh, h_prev_t.T) + self.b_hh[:, np.newaxis]) # TODO
